...
permalink: /p3-string-console-output/
...
**설명:** [ 보충 ]    
• "String".upper()는 클래스의 일부인 메서드 이다.     
• len(strig)는 함수이다.    
• 메서드는 ( . )을 사용한다.    
• 여기서는 여기까지만 이해하고 넘어간다. 향후, Class 를 공부하면 자연히 이해가 된다.
{: .notice--info}
...
#python3
day = 6
print ("03 - {} - 2019".format(day))
# 03 - 6 - 2019
print ("03 - {0:02d} - 2019".format(day))
# 03 - 06 - 2019
```
...
• format 옵션에서 숫자 앞에 0으로 채울려면 {0:02d}와 같이 설정해준다. 
{: .notice--info}
...
#python3
print ("Let's not go to {0}. 'Tis a silly {1}.".format(string_1, string_2))
```    

**설명:** [ Solution ]    
• 1 번째 %s 에는 변수 string_1 이 할당되어 출력된다.    
• 2 번째 %s 에는 변수 string_2 가 할당되어 출력된다.    
• python3는 .format 을 사용하여 출력한다. 
{: .notice--info}
...
#python3
print ("The %s who {0} {1}!".format("Knights", "say", "Ni"))
...
**설명:** [ Solution - Python3 ]    
• {0}, {1}, {2} 순번을 정해주면, (name, quest, color) 순서로 출력됨.    
• name 이 0, quest 가 1, color 이 2 의 자리에 출력됨.     
• {}에 index 값을 지정하지 않으면, 순서대로 자동 출력됨.    
• {}가 입력될 값보다 많으면 Error 발생함. 
{: .notice--info}
